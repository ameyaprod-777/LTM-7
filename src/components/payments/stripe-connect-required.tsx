import Link from "next/link";
import { Landmark, ArrowRight } from "lucide-react";
import { Button } from "@/components/ui/button";
import { PAYOUT_SETTINGS_PATH } from "@/lib/stripe-connect-gate";

type Props = {
  resourceLabel?: string;
};

/** Blocage publication : IBAN (ou Connect) manquant. */
export function StripeConnectRequired({
  resourceLabel = "une annonce",
}: Props) {
  return (
    <div className="rounded-2xl border border-amber-200 bg-amber-50 p-6 sm:p-8">
      <div className="flex items-start gap-3">
        <div className="rounded-xl bg-white p-2 shadow-sm">
          <Landmark className="h-6 w-6 text-accent" />
        </div>
        <div className="min-w-0 flex-1">
          <h2 className="text-lg font-semibold text-anthracite">
            Ajoutez votre IBAN avant de publier
          </h2>
          <p className="mt-2 text-sm text-anthracite-600">
            Pour publier {resourceLabel}, indiquez le compte bancaire (IBAN) sur
            lequel vous souhaitez recevoir vos gains après chaque location. C’est
            rapide : pas besoin de créer un compte Stripe.
          </p>
          <ul className="mt-4 list-inside list-disc space-y-1 text-sm text-anthracite-600">
            <li>IBAN + nom du titulaire</li>
            <li>Virement SEPA après clôture de la location</li>
            <li>Optionnel plus tard : Stripe Connect pour l’automatisation</li>
          </ul>
          <div className="mt-6 flex flex-wrap gap-3">
            <Link href={PAYOUT_SETTINGS_PATH}>
              <Button>
                Renseigner mon IBAN
                <ArrowRight className="ml-2 h-4 w-4" />
              </Button>
            </Link>
            <Link
              href="/dashboard"
              className="inline-flex items-center text-sm text-anthracite-500 hover:text-accent"
            >
              Retour au tableau de bord
            </Link>
          </div>
        </div>
      </div>
    </div>
  );
}
